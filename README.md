...
JanMitra is an AI-powered Ontology & Intelligence Graph platform for Digital Democracy that turns scattered signals—news, citizen reports (with photo evidence), and WhatsApp inputs—into trusted, connected, and actionable civic intelligence.

It solves two real gaps: citizens can’t quickly verify viral claims or track civic issues they report, and governments receive fragmented complaints without a clear way to triage, assign, and ensure accountability. JanMitra fixes this by using AI for fast fact-checking and entity/relationship extraction, structuring everything into an explainable knowledge graph (search + subgraph exploration), and delivering two role-based products: a citizen app for verify/report/track and a government command interface for review/assign/status/analytics.

In short: JanMitra connects truth, participation, and accountability—so citizens trust the system and decision-makers act faster with transparency.

**Note:** We were selected as finalist out of 6.5k+ teams.
...
## Screenshots (Older SS recommends checking out the website)
